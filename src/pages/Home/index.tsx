...
export function HomePage() {
  ...
  const stats = useMemo<DashboardStats>(() => {
    ...
    const activeGoals = goals.filter(
      (goal) => getGoalDisplayStatus(goal) === "active"
    ).length;
    const completedGoals = goals.filter(
      (goal) => getGoalDisplayStatus(goal) === "completed"
    ).length;
        ...
        <Paper className="dashboard-panel" radius="xl" p="lg">
          <Group justify="space-between">
            ...
            <Badge variant="light" color="blue">
              {recentTodoTasks.length} 条
            </Badge>
          </Group>

          {recentTodoTasks.length > 0 ? (
            <Stack className="dashboard-task-list" gap="sm">
              {recentTodoTasks.map((task) => (
                <Card
                  key={task.id ?? `${task.goal_id}-${task.title}`}
                ...
                >
                  <Group
                    ...
                    align="flex-start"
                    className="dashboard-task-head"
                  >
                    <Text fw={700} c="dark.7">
                      {task.title || "未命名任务"}
                    </Text>
                    <Badge
                      className={`dashboard-task-status is-${
                        task.status ?? "todo"
                      }`}
                      variant="light"
                    >
          ...
          {recentGoals.length > 0 ? (
            <Stack className="dashboard-goal-list" gap="sm">
              {recentGoals.map((goal) => (
                ...
                >
                  <Badge
                    className={`dashboard-task-status dashboard-goal-status ${getGoalStatusBadgeClass(
                      getGoalDisplayStatus(goal)
                    )}`}
                    variant="light"
                  >
