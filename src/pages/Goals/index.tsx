...
import { getGoalDisplayStatus, getGoalStatusLabel, goalStatusOptions } from '../../common/dicts'
import { formatDateTime } from '../../common/utils/date'
import './index.css'

type GoalFormState = Required<
  Pick<GoalCreateGoalRequest, 'title' | 'description' | 'category' | 'target_deadline'>
>
...
export function GoalsPage() {
  ...
  const [activeDetailTab, setActiveDetailTab] = useState<GoalDetailTab>('basic')
  ...
  async function loadGoalList(nextSelectedId?: number | null) {
    setIsListLoading(true)

    try {
      const response = await goalsApi.goalsList()
      const nextList = response.data.data?.list ?? []
      const resolvedSelectedId =
        nextSelectedId ?? selectedGoalId ?? nextList[0]?.id ?? null
  ...
  async function loadGoalDetail(goalId: number) {
    ...
    try {
      ...
      message.error(error instanceof Error ? error.message : '获取目标详情失败。')
    } finally {
  ...
  async function loadGoalPlan(goalId: number) {
    ...
    try {
      ...
      if (!isPlanNotFoundError(error)) {
        message.error(error instanceof Error ? error.message : '获取目标计划失败。')
      }
    } finally {
  ...
  async function handleCreateGoal(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await loadGoalList(createdGoalId)
      setIsCreateModalOpen(false)
  ...
  async function handleGenerateGoalPlan() {
    ...
    try {
      ...
      message.success(selectedPlan ? '目标计划已重新生成。' : '目标计划已生成。')
    } catch (error) {
      message.error(
          ...
          <Paper className="goals-panel goals-list-panel" radius="xl" p="lg">
            ...
            {goalList.length > 0 ? (
                ...
                {goalList.map((goal) => {
                  ...
                  return (
                    <UnstyledButton
                      ...
                      className={`goals-list-item${isActive ? ' is-active' : ''}`}
                      onClick={() => {
                        if (!goal.id) {
                          return
                        }
                    ...
                    >
                      ...
                      <Text size="sm" c="dimmed">{getGoalStatusLabel(getGoalDisplayStatus(goal))}</Text>
                    </UnstyledButton>
                  )
                })}
              </Stack>
              ...
              <div className="goals-empty-state">
                <Text c="dimmed">{isListLoading ? '目标加载中...' : '暂无目标'}</Text>
              </div>
            )}
          </Paper>
        </div>
        ...
        <Paper className="goals-panel goals-detail-panel" radius="xl" p="lg">
          ...
          {selectedGoal ? (
            <Tabs
              ...
              onChange={(value) => setActiveDetailTab((value as GoalDetailTab) ?? 'basic')}
            >
              <Tabs.List>
              ...
              <Tabs.Panel value="basic" pt="md">
                <div className="goal-detail">
                  <Group className="goal-detail-meta" gap="md" wrap="wrap">
                    <Paper className="goal-detail-item" radius="lg" p="md" withBorder>
                      <Text size="sm" c="dimmed">ID</Text>
                      <Text fw={700}>{selectedGoal.id ?? '-'}</Text>
                    </Paper>
                    <Paper className="goal-detail-item" radius="lg" p="md" withBorder>
                      <Text size="sm" c="dimmed">创建时间</Text>
                      <Text fw={700}>{formatDateTime(selectedGoal.created_at)}</Text>
                    </Paper>
                    <Paper className="goal-detail-item" radius="lg" p="md" withBorder>
                      <Text size="sm" c="dimmed">更新时间</Text>
                      <Text fw={700}>{formatDateTime(selectedGoal.updated_at)}</Text>
                    </Paper>
                  </Group>

                  <form className="goal-detail-form" onSubmit={handleUpdateGoal}>
                    <Stack gap="md">
                      <TextInput
                        label="标题"
                        value={editForm.title ?? ''}
                        onChange={(event) => updateEditField('title', event.currentTarget.value)}
                        placeholder="请输入目标标题"
                      ...
                      <Textarea
                        ...
                        onChange={(event) => updateEditField('description', event.currentTarget.value)}
                        placeholder="请输入目标描述"
                        disabled={!isEditingGoal}
                      />
                      ...
                      <div className="goal-detail-grid">
                        <TextInput
                          ...
                          onChange={(event) => updateEditField('category', event.currentTarget.value)}
                          placeholder="请输入目标分类"
                          disabled={!isEditingGoal}
                        />
                        ...
                        <TextInput
                          ...
                          onChange={(event) =>
                            updateEditField('target_deadline', event.currentTarget.value)
                          }
                          placeholder="请输入截止时间"
                          disabled={!isEditingGoal}
                        />
                  ...
                  <form className="goal-status-form" onSubmit={handleUpdateGoalStatus}>
                    <Group align="end">
                      <Select
                        className="goals-field"
                        label="状态"
              ...
              <Tabs.Panel value="plan" pt="md">
                <div className="goal-plan-panel">
                  <Group justify="space-between" className="goals-panel-header goal-plan-header">
                    ...
                    >
                      {isPlanGenerating ? '生成中...' : selectedPlan ? '重新生成计划' : '生成计划'}
                    </Button>
                  </Group>
                  ...
                  {selectedPlan ? (
                    <Stack className="goal-plan-detail" gap="md">
                      <Paper className="goal-detail-item" radius="lg" p="md" withBorder>
                        <Text size="sm" c="dimmed">计划标题</Text>
                        <Text fw={700}>{selectedPlan.title || '-'}</Text>
                      </Paper>
                      <Paper className="goal-detail-item" radius="lg" p="md" withBorder>
                        <Text size="sm" c="dimmed">计划概述</Text>
                        <Text>{selectedPlan.overview || '-'}</Text>
                      </Paper>
                      <div className="goal-detail-grid">
                        <Paper className="goal-detail-item" radius="lg" p="md" withBorder>
                          <Text size="sm" c="dimmed">计划ID</Text>
                          <Text fw={700}>{selectedPlan.id ?? '-'}</Text>
                        </Paper>
                        <Paper className="goal-detail-item" radius="lg" p="md" withBorder>
                          <Text size="sm" c="dimmed">计划创建时间</Text>
                          <Text fw={700}>{formatDateTime(selectedPlan.created_at)}</Text>
                        </Paper>
                      </div>
                    </Stack>
                  ) : (
                    <div className="goals-empty-state goals-plan-empty-state">
                      <Text c="dimmed">
                        {isPlanLoading ? '计划加载中...' : '当前目标还没有计划，点击上方按钮生成。'}
                      </Text>
                    </div>
                  )}
                </div>
            ...
            <div className="goals-empty-state">
              <Text c="dimmed">{isDetailLoading ? '详情加载中...' : '请选择目标'}</Text>
            </div>
          )}
        </Paper>
      </section>
        ...
        <form className="goals-form" onSubmit={handleCreateGoal}>
          <Stack gap="md">
            ...
            <TextInput
              ...
              onChange={(event) => updateField('target_deadline', event.currentTarget.value)}
